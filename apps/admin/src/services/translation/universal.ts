...
export class UniversalTranslator implements TranslationService {
	...
	constructor(adapter: TranslationAdapter, fallbackAdapter?: TranslationAdapter) {
	...
	async translateText(text: string, from: string, to: string): Promise<string> {
		...
		try {
			...
			if (this.fallbackAdapter?.isAvailable()) {
				console.warn(`Translation failed with ${this.adapter.name}, trying fallback:`, error)
				try {
					return await this.fallbackAdapter.translateText(text, from, to)
				} catch (fallbackError) {
			...
			console.error(`All translation attempts failed for "${text.substring(0, 50)}...":`, error)
			return text
		}
	}

	async translateRichText<T>(content: T, from: string, to: string): Promise<T> {
		...
		if (content && typeof content === "object") {
			...
			for (const [key, value] of Object.entries(content)) {
				if (key === "text" && typeof value === "string") {
					...
					translated[key] = await this.translateRichText(value, from, to)
				}
			}
