...
There is no hosted test environment for this feature. Development configuration is for the local admin process. Its evaluation model URL is `http://127.0.0.1:8766/model.bin`, which requires the local model server to be running. Do not use this loopback address for Production. For the initial production rollout, the owner selected `https://www.calligrapher.ai/d.bin` as the server-side model source. Its bytes were verified against the SDK's pinned SHA-256 on September 21, 2026. The owner subsequently requested a private Blob copy. On the first uncached generation in each environment, Admin imports the verified bytes into `handwriting/models/<model-sha256>.bin` in that environment's existing private handwriting store. Later cold starts read and verify this private copy; they do not contact the original source. A storage error or corrupted private file fails closed instead of falling back upstream. The import URL can be removed after the copy has been verified; retain it only if automatic re-import after a missing model is desired. No public model endpoint is added.
...
- The pinned model was imported into the private Development store. A subsequent real Blob read succeeded after the import URL was cleared from the test process, confirming that stored-model loading does not require the original source.
