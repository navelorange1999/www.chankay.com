...
The authorized trial uses the existing Trading article and a generated cover for the personal Official Account. Publication API permission is unavailable; the trial must stop at draft synchronization. The Preview account has been configured and the cover uploaded. Preview deployment `3xwZXdMFN9651d63bGygxg1A1DH8` is Ready for commit `e526151`. Remote draft verification remains pending operator login on the new Preview hostname and live API execution. No WeChat mutation has occurred.
