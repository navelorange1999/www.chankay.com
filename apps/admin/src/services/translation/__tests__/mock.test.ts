...
describe("MockTranslationAdapter", () => {
	...
	describe("translateText", () => {
		it("should translate text with language prefix", async () => {
			const result = await translator.translateText("Hello world", "en", "zh-CN")
	...
	describe("translateRichText", () => {
		it("should translate string content", async () => {
			const result = await translator.translateRichText("Hello", "en", "zh-CN")
		...
		it("should handle null and undefined content", async () => {
			expect(await translator.translateRichText(null, "en", "zh-CN")).toBeNull()
			expect(await translator.translateRichText(undefined, "en", "zh-CN")).toBeUndefined()
		})

		it("should preserve non-object/array content", async () => {
			expect(await translator.translateRichText(123, "en", "zh-CN")).toBe(123)
			expect(await translator.translateRichText(true, "en", "zh-CN")).toBe(true)
		})
	})
})
